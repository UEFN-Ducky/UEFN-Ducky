...
describe("railResizeEnd", () => {
  it("closes when the handle is dragged past the collapse width", () => {
    expect(railResizeEnd(40, 180, 560)).toEqual({ close: true });
  });

  it("snaps back to min when released between collapse and min", () => {
    expect(railResizeEnd(120, 180, 560)).toEqual({ close: false, width: 180 });
  });

  it("keeps an in-range width", () => {
    expect(railResizeEnd(240, 180, 560)).toEqual({ close: false, width: 240 });
  });
});
