...
def _ungate_plugin_tools(monkeypatch):
    ...
    monkeypatch.setattr("backend.tools.support.plugin_gate.require_plugin", lambda pid: None)
...
def test_empty_index(isolated_appdata, tmp_path):
    from backend.memory.project import index_markdown, list_entries
...
def test_sub_entry_without_parent_creates_stub_main(isolated_appdata, tmp_path):
    from backend.memory.project import read_entry, save_entry
...
def test_delete_nested(isolated_appdata, tmp_path):
    from backend.memory.project import delete_entry, list_entries, read_entry, save_entry
...
def test_deep_nesting_rejected(isolated_appdata):
    from backend.memory.project import slugify_entry_name
...
def test_same_named_projects_stay_separated(isolated_appdata, tmp_path):
    ...
    from backend.memory.project import list_entries, save_entry
...
def test_append_creates_then_extends(isolated_appdata, tmp_path):
    from backend.memory.project import append_entry, read_entry
...
def test_delete_entry(isolated_appdata, tmp_path):
    from backend.memory.project import delete_entry, list_entries, save_entry
...
def test_writes_own_project_reads_any(two_projects):
    ...
    from backend.memory.project import list_entries
    from backend.tools.uefn.memory import project_memory_get, project_memory_list, project_memory_save
...
def test_tools_roundtrip_active_project(two_projects):
    from backend.tools.uefn.memory import (
...
def test_memory_overview_spans_projects(two_projects):
    ...
    from backend.memory.project import save_entry
    from backend.tools.uefn.memory import ducky_memory_overview
