...
from backend.listener_status import ListenerStatusState, fetch_listener_status


def test_busy_listener_stays_online(monkeypatch):
    monkeypatch.setattr(
        "backend.listener_status.listener_get_health",
    ...
    monkeypatch.setattr("backend.listener_status.ping_listener", _fail_ping)
...
def test_ping_failure_keeps_online_and_wedges_after_streak(monkeypatch):
    ...
    monkeypatch.setattr(
        "backend.listener_status.ping_listener",
        lambda *_a, **_k: (False, None),
    )
    # No recent successful POST — the wedge probe must actually ping.
    monkeypatch.setattr(
        "backend.listener_status.seconds_since_last_post_ok",
        lambda: float("inf"),
    )
    monkeypatch.setattr(
        "backend.listener_status.listener_project_fields",
...
def test_get_failure_is_offline(monkeypatch):
    monkeypatch.setattr("backend.listener_status.listener_get_health", lambda _port: None)
    monkeypatch.setattr(
        "backend.listener_status.ping_listener",
...
def test_recent_agent_command_skips_status_ping(monkeypatch):
    ...
    monkeypatch.setattr("backend.listener_status.ping_listener", _fail_ping)
    monkeypatch.setattr("backend.listener_status.seconds_since_last_post_ok", lambda: 2.0)
    monkeypatch.setattr(
        "backend.listener_status.listener_project_fields",
        lambda *_a, **_k: ("", "", True, None),
    )
