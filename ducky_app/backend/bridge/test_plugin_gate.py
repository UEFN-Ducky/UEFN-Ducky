...
def test_call_tool_clears_stale_cache_for_missing_name() -> None:
    ...
    prev_list = server.request_handlers.get(ListToolsRequest)
    prev_call = server.request_handlers.get(CallToolRequest)
    assert prev_list is not None and prev_call is not None, "FastMCP handlers missing"
...
def test_run_bridge_still_reaches_mcp_run_fast() -> None:
    ...
    with (
        ...
        patch("backend.bridge.plugin_gate.install_bridge_plugin_gate", lambda *_a, **_k: None),
    ):
