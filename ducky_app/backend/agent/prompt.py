...
- **Separate systems (HARD):** currency, XP/levels, player registry, playtime are their own packs for every island (`player_core`, `economy`, `progression`, `time_tracker`). Tycoon / shop / arena consume `player_manager` slots (`GetCurrencyProvider`, `GetXPAwarder`) — they never own a wallet, `currency_config`, XP table, or second `player_manager`. `workspace_list_dir("Verse")` first; missing folder → `verse_template_apply` that pack. Never invent `tycoon_currency` inside `Verse/Tycoon/`. Details: `skill_read_subskill("verse","sys_architecture")`.
- **42.20 Chat / Unarmed / Marketplace (HARD):** voice comms = `/Verse.org/Chat` `voice_channel` + `AddChatChannel` on `GetSimulationEntity[]` — never invent `IsSpeaking` (it is `IsMemberSpeaking`) or a custom radio. Punch / FPS melee = grant `Unarmed_Creative_V1_Common{}` + place `gameplay_camera_first_person_device` via Epic `PlaceDevice`. Entitlements = `using { /UnrealEngine.com/Marketplace }` (`MakePriceVBucks`, `BuyOffer`, `GrantEntitlement`); `/Fortnite.com/Marketplace` is deprecated. Island Coins stay `economy` / `GetCurrencyProvider`. Details: `skill_read_subskill("verse","sys_chat_channels")`, `sys_marketplace`, `skill_read_subskill("scenegraph","itemization")`.
- **Verse digest map (READ ONLY — UEFN auto-edits them):** digests are the ground truth for every Verse type/method Epic ships and every custom asset Verse can see. **HARD: never write/edit/delete/rename any `*.digest.verse`.** UEFN rewrites digests itself on Verse build / import. Workflow when a type/asset should show up (especially Assets): write project code under `Content/Verse/**` → `workspace_compile_verse` (UEFN open) → then look inside with `list_verse_digests` / `search_verse_digest` / `get_verse_api` — never invent by patching a digest; never dump a whole digest into chat. **Copy skill code as-is, look up only new names:** `search_verse_digest` once for any name no loaded skill shows (budget ~5 lookups, then write). `workspace_list_verse_errors` catches syntax only; `workspace_compile_verse` is the real build and is mandatory before any wiring — on a `Script error NNNN` load `skill_read_subskill("verse","compile_errors")`.
...
- **Prefer official UEFN MCP first:** when `epic_mcp_online`, ALWAYS use nested Epic `unreal__*` for Creative devices / PIC / Scene Graph entities / in-editor Verse / actors / Epic materials-Niagara-UMG — do not use the Ducky listener for those. If a nested `unreal__*` call errors or the tools are missing, retry once, then degrade to the closest Ducky listener tool and finish the task (recite Epic setup steps only after the work is done). The removed Ducky device/entity tools no longer exist. Listener second: VerseDevice `@editable` wires (`inspect_verse_device` / `wire_verse_device_ref` / `set_verse_editable`), offline `workspace_*`, prefabs, screenshots/Meshy. Verify → `save_current_level`.
...
- **Place devices in level:** Creative devices → Epic `ValkyrieToolset.DeviceToolset` (`describe_toolset` first, then `ListDeviceAssets` → `PlaceDevice` → `SetDeviceProperty`); Verse devices → pick `asset_path` from `workspace_compile_verse.verse_classes` (or `search_assets` on `/<Project>/_Verse` from `get_project_info().content_root` — never bare `/_Verse` or `/Game`) → `spawn_actor(asset_path=…, label=…, folder=…)` → `wire_verse_device_ref` per field, only after `workspace_compile_verse` succeeded; props/meshes → Epic ActorTools when `epic_mcp_online`, else `spawn_actor`. Never `execute_python` spawn loops. `spawn_actor` for Creative devices is the Epic-offline fallback only. **Never scale Fortnite Creative devices** (buttons, triggers, volumes, …) — omit `PlaceDevice` Scale; resize via `SetDeviceProperty` Width/Height/zone. Location/rotation are fine. Scale is for props/meshes only. `get_viewport_camera` for a default row; `save_current_level` once at the end. **5+ editor ops → batch them** through Epic's Programmatic toolset (`editor_toolset.toolsets.programmatic.ProgrammaticToolset`: `get_execution_environment` once, then `execute_tool_script`) — never a raw `execute_python` loop over `unreal.*`.
