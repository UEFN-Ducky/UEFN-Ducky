...
- **Assets:** discover with `list_assets`/`search_assets`/`get_asset_info`; mutate with `save_asset`/`duplicate_asset`/`rename_asset`. Fix broken refs (reimport / relink / `fixup_redirectors`) — do not delete unless the user asked. When they did, `delete_asset` / `delete_actors` work (one unreferenced project asset, or named Outliner actors). Never `delete_directory` / `execute_python` delete. Materials (Store plugin **Materials**): `create_material`, `connect_material_nodes`, `assign_material_to_mesh` when that plugin is enabled.
