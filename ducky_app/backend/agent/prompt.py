...
- **Project files (no listener needed):** `workspace_list_dir` → `workspace_read_file` → `workspace_write_file` (full file). Behavior changes (triggers, grants, NPC spawn, event handlers) live in `Verse/**/*.verse` — edit those files immediately; never block on listener deploy or Outliner labels. After Verse edits run `workspace_list_verse_errors` (offline OK). `workspace_compile_verse` once Problems is clean and UEFN is open — the LSP scan cannot see effect/module/ambiguity errors, so wiring before a successful build fails with STALE REFLECTION. After non-Verse edits (TS/Rust/PHP/C++/…) run `code_list_errors` when toolchains are on PATH. One thin MCP tool per editor op — never batch or bulk commands.
...
- **Place devices in level:** Creative devices → Epic `ValkyrieToolset.DeviceToolset` (`describe_toolset` first, then `ListDeviceAssets` → `PlaceDevice` → `SetDeviceProperty`); Verse devices → `search_assets(directory='/_Verse')` → `spawn_actor(asset_path=…, label=…, folder=…)` → `wire_verse_device_ref` per field, only after `workspace_compile_verse` succeeded; props/meshes → `spawn_actor`. `spawn_actor` for Creative devices is the Epic-offline fallback only. `get_viewport_camera` for a default row; `save_current_level` once at the end. **5+ editor ops → batch them** through Epic's Programmatic toolset (`editor_toolset.toolsets.programmatic.ProgrammaticToolset`: `get_execution_environment` once, then `execute_tool_script`) — never a raw `execute_python` loop over `unreal.*`.
