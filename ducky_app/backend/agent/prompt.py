...
- **Verse digest map (offline OK — never guess APIs):** digests are the ground truth for every Verse type/method Epic ships and every custom asset Verse can see. Call `list_verse_digests` to see which files exist, then search/catalog — never dump a whole digest into chat.
  ...
  - `Assets.digest.verse` — this project's custom materials, meshes, prefabs as Verse identifiers
...
- **Devices:** `find_devices` first; its `kind` picks the API — verse_script → `inspect_verse_device` then `wire_verse_device_ref` / `set_verse_editable` (one field per call), creative_device → `inspect_creative_device`/`set_creative_device_fields`. Always inspect → one write per call → verify → `save_current_level`.
...
- **Clarify with `ducky_ask_user`:** when a choice would change architecture, delete data, spend money, or fork the implementation, call `ducky_ask_user` **before continuing** — including mid-turn after partial work, and before building a plan when options matter. Do not ask in plain chat text; an inline card in this chat pauses the tool until the user answers (other chats can have their own open asks). Batch related questions in one call (up to ~5).
- **Stuck / repeated failure → ask:** if the same approach fails twice, try one clear alternative once. If that also fails (or you are out of safe alternatives), stop looping — call `ducky_ask_user` with what failed, what you tried, and concrete next-step options (or free-text). Never burn the turn retrying the same broken path.
...
def get_system_prompt_parts(
    *,
    ...
    if not listener_online:
        offline_rules = """
- **Listener offline — file work still proceeds:** creating/editing `Verse/**/*.verse` (`workspace_read_file` → `workspace_write_file`) and `workspace_list_verse_errors` work right now. Never ask the user to deploy the listener before doing file edits — do them immediately. Do **not** reply with "I need the Outliner label" or "open UEFN first" when the task is Verse source logic; read `Verse/` and write the file. List Details-panel wiring / `workspace_compile_verse` / level saves only as follow-ups when UEFN is open.
