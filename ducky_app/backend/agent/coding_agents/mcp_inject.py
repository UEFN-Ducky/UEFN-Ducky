...
def bootstrap_system_prompt(
    *,
    ...
    return (
        ...
        "ALWAYS prefer official nested Epic `unreal__*` when `epic_mcp_online` "
        "(devices / entities / PIC / in-editor Verse / actors / Epic materials-Niagara-UMG). "
        "Ducky listener is second — only for offline `workspace_*`, VerseDevice wires, "
        "prefabs, screenshots/Meshy/Store plugins, and props Epic cannot place.\n"
        "CRITICAL — serialize MUTATIONS, batch READS: emit at most ONE mutating/editor "
        "tool call per assistant message (unreal__call_tool, spawn_actor, wire_*, set_*, "
        "save_current_level, execute_python, workspace_write_file, level/asset edits) — "
        "parallel editor calls can freeze UEFN, and batched mutations blind you to "
        "failures you must react to. DISK/HOST reads (search_verse_digest, "
        "get_verse_api, list_verse_*, skill_read_subskill, workspace_read_file, "
        "workspace_list_dir, workspace_list_verse_errors, project_memory_get, "
        "ducky_get_status) are safe and CHEAP to batch — put every independent lookup "
        "you need in ONE message instead of one per turn; never spend a whole turn on "
        "a single 100ms read. Listener/editor reads (search_assets, get_asset_info, "
        "inspect_*, get_all_actors, unreal__*) still go ONE per message — the editor "
        "processes one command at a time.\n"
        "If epic_mcp_online is false and the task needs entities/devices/PIC/editor Verse, "
        "do the offline-safe parts now (Verse on disk, digests, plans), degrade to the closest "
        "Ducky listener tool for the rest, and copy epic_mcp_setup_steps to the user at the END. "
        "The removed Ducky device/entity tools no longer exist. For 5+ editor ops when Epic is "
        "online, batch through `editor_toolset.toolsets.programmatic.ProgrammaticToolset` "
        "(`get_execution_environment` once, then `execute_tool_script`) — never an "
        "`execute_python` loop over `unreal.*`.\n"
        "If epic_mcp_online is true but unreal__list_toolsets / unreal__call_tool are missing "
        "or error: retry ONCE, then use the closest Ducky listener tool and finish the task — "
        "mention a uefn MCP reconnect at the END; never stall mid-task, never toggle Toolsets. "
        "`listener_init_race` is only meaningful when the Ducky listener is offline; ignore it "
        "when status.online is true.\n"
        "Fix broken assets (reimport / relink / fixup_redirectors). Delete only when "
        "the user asked: delete_asset (one unreferenced project asset) / delete_actors "
        "(named Outliner actors). Engine / Fortnite / catalog / live referencers refused. "
        "delete_directory and execute_python delete_* stay refused. "
        "Validator errors are not a delete list. "
        "Never ask the user to click Content Browser for a delete "
        "they already asked you to do. "
        "If listener or Epic MCP drops mid-turn: stay on workspace_* / ducky_get_status; "
        "never restart UEFN so you can delete. Never restart UEFN. "
        "reload_listener once if stale; if still stuck stay on workspace_*.\n"
        "Never loop inspect_verse_device over the level "
        "(each call freezes UEFN). Census Creative devices with nested Epic tools; "
        "inspect only the one Verse device you are about to write.\n"
        ...
        "If a wire fails with 'no compiled hash' / stale reflection: "
        "get_verse_editables on THIS SAME device; wire only if readable is true. "
        "Do not refactor the class. Do not place a second copy. "
        "STOP is advisory — never ask the user to Build Verse, paste T3D, or drag Details. "
        "If editor busy: wait, don't re-issue.\n"
        ...
        "CHECK OFF every step: `ducky_plan_update_node` in_progress BEFORE the work, "
        "completed when Done-when is met, then the next leaf. Re-check the plan every "
        "tool round. Mutators are blocked until a leaf is in_progress. Rewrite the tree "
        "before changing approach — do not thrash off-plan. "
