...
AGENT_HARD_RULES = (
    ...
    "- **Inspect is not a census (HARD):** `find_devices` once for labels / "
    "`kind` / `script_class`. Never loop `inspect_verse_device` or "
    "`inspect_creative_device` over the level — each call hits the UEFN game "
    "thread and freezes the editor. Inspect only the ONE device you are about "
    "to write. Verse class names are `find_devices[].script_class`; field names "
    "come from `workspace_read_file` of that class's `.verse`.\n"
    ...
    "the Verse VM relinked. `add_entity_component` returning `class_path` under "
    "`/Engine/Transient` with `VERSE_DEAD_*` is a stale class — `reload_listener`, "
    "then destroy and recreate the entity. Digest deadlock (`Script error 9002: "
