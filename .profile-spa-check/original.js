/* SPA /profile: park once. Named pair + live dots. No history wrap. */
(function () {
  ...
  function paintAcc(scope) {
    (scope || document).querySelectorAll(".b-account-security").forEach(function (card) {
      if (card.getAttribute("data-uefn-acc-init") === "1") {
        if (card.getAttribute("data-uefn-acc-user") === "1") {
          decorateSecurity(card);
        }
        return;
      }
  ...
  function watchUntil(root, pred, done, ms) {
    ms = ms || 3000;
    if (pred()) {
      done();
      return;
  ...
  function watchLayoutOnce() {
    var root = document.getElementById("app-content");
    if (!root) return;
    if (root.getAttribute("data-uefn-layout-watch") === "1") {
      if (document.querySelector(".user-profile-layout")) {
        parkLogout();
        parkLang();
        paintIdentity();
      }
    ...
    root.setAttribute("data-uefn-layout-watch", "1");
    watchUntil(
      root,
      function () {
        return !!document.querySelector(".user-profile-layout");
      },
      function () {
        parkLogout();
        parkLang();
  ...
  function watchSecurityOnce() {
    function go(card) {
      decorateSecurity(card);
    }
    var card = document.querySelector(".b-account-security");
    var root = document.getElementById("app-content");
    if (!card) {
      if (!root || root.getAttribute("data-uefn-sec-find") === "1") return;
      root.setAttribute("data-uefn-sec-find", "1");
      watchUntil(
        root,
        function () {
          return !!document.querySelector(".b-account-security");
        },
        function () {
          watchSecurityOnce();
        },
    ...
    if (card.querySelector(".registry-tabs")) {
      go(card);
      return;
    }
    if (card.getAttribute("data-uefn-sec-watch") === "1") return;
    card.setAttribute("data-uefn-sec-watch", "1");
    watchUntil(
      card,
      function () {
        return !!card.querySelector(".registry-tabs");
      },
      function () {
        go(card);
      },
  ...
  function tickPcs(list) {
    parkAddCard();
    paintRemoveX(list);
    parkDots();
    return !!(list && list.querySelector(".ud-remote__pc--add"));
  }
  function watchPcsOnce() {
    var root = document.getElementById("app-content");
    if (!root) return;
    if (root.getAttribute("data-uefn-pcs-life") === "1") {
      tickPcs(document.querySelector(".ud-remote__pcs"));
      return;
    }
    root.setAttribute("data-uefn-pcs-life", "1");
    tickPcs(document.querySelector(".ud-remote__pcs"));
    var obs = new MutationObserver(function () {
      tickPcs(document.querySelector(".ud-remote__pcs"));
    });
  ...
  function watchAll() {
    if (document.documentElement.getAttribute("data-uefn-route-watch") === "13") return;
    document.documentElement.setAttribute("data-uefn-route-watch", "13");
